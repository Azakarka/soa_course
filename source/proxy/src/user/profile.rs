...
pub async fn get_profile_info(req: HttpRequest, config: web::Data<ProxyConfig>) -> impl Responder {
    ...
    let jwt = match req.cookie("jwt-token") {
        ...
        None => {
            return HttpResponse::Unauthorized().body("No jwt token specified")
        }
    };
...
#[post("/profile")]
pub async fn update_profile(req: HttpRequest, update_message: web::Json<UpdateMessage>, config: web::Data<ProxyConfig>) -> impl Responder {
    debug!("New Update");
    let jwt = match req.cookie("jwt-token") {
        Some(token) => {
           token.value().to_string()
